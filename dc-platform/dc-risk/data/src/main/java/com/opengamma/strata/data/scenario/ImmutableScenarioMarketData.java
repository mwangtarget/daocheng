...
public final class ImmutableScenarioMarketData
  ...
  public ScenarioMarketData combinedWith(ScenarioMarketData other) {
    ...
    ImmutableScenarioMarketData otherData = (ImmutableScenarioMarketData) other;

    if (scenarioCount != 1 && otherData.scenarioCount != 1 && scenarioCount != otherData.scenarioCount) {
      throw new IllegalArgumentException(
          Messages.format(
              ...
              scenarioCount,
              otherData.scenarioCount));
    }
    int mergedCount = Math.max(scenarioCount, otherData.scenarioCount);
    ...
    Map<MarketDataId<?>, MarketDataBox<?>> values = new HashMap<>(otherData.values);
    values.putAll(this.values);
    Map<ObservableId, LocalDateDoubleTimeSeries> timeSeries = new HashMap<>(otherData.timeSeries);
     ...
     */
    private static final class Builder extends DirectFieldsBeanBuilder<ImmutableScenarioMarketData> {
